...
def do_define_form(expressions, env):
    ...
    if scheme_symbolp(signature):
        ...
        # BEGIN PROBLEM 4
        rests=scheme_eval(expressions.rest.first, env)
        '''print("DEBUG:",expressions.rest,expressions.rest.first)'''
        env.bindings[signature]=rests
        return signature
        # END PROBLEM 4
    elif isinstance(signature, Pair) and scheme_symbolp(signature.first):
        # defining a named procedure e.g. (define (f x y) (+ x y))
        # BEGIN PROBLEM 10
        lam=do_lambda_form(Pair(signature.rest,expressions.rest),env)
...
def make_let_frame(bindings, env):
    ...
    while bindings is not nil:
        ref=bindings.first
        '''print(ref,ref.first,ref.rest.first)'''
        validate_form(ref,2,2)
        names=Pair(ref.first,names)
        vals=Pair(scheme_eval(ref.rest.first, env),vals)
