...
def swine_align(player_score, opponent_score):
    ...
    """
    # BEGIN PROBLEM 4a
    for i in range(1,player_score+opponent_score):
        if i> player_score or i> opponent_score:
            return False
        if player_score % i==0 and opponent_score % i ==0 and i>=10:
            return True
    # END PROBLEM 4a
    ...
    def f(s0,s1):
        if who==0:
            if s0-last_score>running_high:
                print(s0-last_score,"point(s)! The most yet for Player",who)
                return announce_highest(who,s0,s0-last_score)
            return announce_highest(who,s0,running_high)
        else:
            """print("DEBUG:",running_high,s1-last_score)"""
            if s1-last_score>running_high:
                print(s1-last_score,"point(s)! The most yet for Player",who)
...
def max_scoring_num_rolls(dice=six_sided, trials_count=1000):
    ...
    """
    # BEGIN PROBLEM 9
    fun0=make_averaged(roll_dice,trials_count)
    max=0
    count=1
    for i in range(1,11):
        fun=fun0(i,dice)
