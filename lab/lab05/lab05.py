...
def closer_city(lat, lon, city_a, city_b):
    ...
    return get_name(city_a) if distance(city_a,now)<distance(city_b,now) else get_name(city_b)
...
def riffle(deck):
    ...
    """
    "*** YOUR CODE HERE ***"
    return [deck[i//2] if (i+1)%2 else deck[len(deck)//2+i//2] for i in range(len(deck))]
...
def add_trees(t1, t2):
    ...
    # Lengths of each tree's branches
    length_t1 = len(t1_branches)
    length_t2 = len(t2_branches)
    max_length = max(length_t1, length_t2)

    new_branches = []

    # Iterate over the longest list of branches
    for i in range(max_length):
        b1 = t1_branches[i] if i < length_t1 else None
        b2 = t2_branches[i] if i < length_t2 else None
        if b1 and b2:
